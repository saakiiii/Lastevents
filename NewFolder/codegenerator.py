...
class codeId:
    ...
    def generate_id(self):
        code_ = None
        x = 100
        response = requests.request("POST", self.findone, headers=self.header, data=self.payload).json()
        ids = response["document"]["ids"]
        while True:
            endnum = 99999+len(ids)+x
    ...
    def generate_image_filename(self):
        ...
        while True:
            endnum = 99+len(ids)+x
            ascii_letters = string.ascii_letters
            number = random.randint(1, endnum)
            letters = [ascii_letters[i] for i in range(len(ascii_letters))]
            random.shuffle(letters)
            random_letters = letters[:3]
            time = datetime.datetime.now().strftime("%H:%M:%S").split(':')
            random_letters.extend(time)
            first_half = "".join(random_letters)
            code = first_half+str(number)
    ...
    def generate_post_id(self):
        ...
        random.shuffle(lts)
        w = "".join(lts)
        return f"pst{w}{random.randint(100, 9999)}"
                
    def generate_page_id(self):            
        lts = ["P", "A", "G", "E"]
        random.shuffle(lts)
        w = "".join(lts)
        return f"pge{w}{random.randint(100, 9999)}"