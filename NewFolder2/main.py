    ...
    if request.method == 'GET':
       ...
       if current_user.is_authenticated:
            ...
            for i in range(len(articles_news)):
                if i % 2 == 0:
                    articles_news[i]["total_words"] = 89
                else:
                    articles_news[i]["total_words"] = 110
...
def blog_read():
    ...
    if request.method == 'GET':
        ...
        if current_user.is_authenticated:
            notifications = be.getNotifcations(user_id=current_user.user_id)
            return render_template("blog.html", response=response, logged_in = current_user.is_authenticated, title=title, metadata=metadata, blog__="#8479794f", notifications=notifications)
...
def view_news_article(news_id_or_news_heading):
    ...
    if current_user.is_authenticated: 
     notifications = be.getNotifcations(user_id=current_user.user_id)
     return render_template("view.html", response=news, news=True, logged_in = current_user.is_authenticated, data=data, title=title, notifications=notifications)
...
def view_blogs_article(blog_id_or_blogs_heading):
    ...
    if current_user.is_authenticated:
     notifications = be.getNotifcations(user_id=current_user.user_id)
     return render_template("view.html", response=blog, blog=True, logged_in = current_user.is_authenticated, data=data[0:10], title=title, metadata=metadata, read__="#8479794f", notifications=notifications)
    return render_template("view.html", response=blog, blog=True, logged_in = current_user.is_authenticated, data=data[0:10], title=title, metadata=metadata, read__="#8479794f")
...
def news_get_country_state_category(country, state, category, city):
    ...
    for i in range(len(response)):
        if i % 2 == 0:
         response[i]["total_words"] = 89
        else:
         response[i]["total_words"] = 110
...
@app.route('/home')
def home():
    home_meta = '''Space for reading, writing news, and earning money from your news and blog articles. We are providing you with news updates and the last events to keep you updated. We are having the
    space to earn money here through your articles. Here you find the latest events as soon as possible. In the writing space, you can write your news articles and start earning.'''
    title = "Space for sharing information about the events and updating yourself with the latest events(last events)"
    notifications = None
...
@app.route('/develop')
def main():
    return render_template("base copy.html", __me__=True)
    ...
    if request.method == 'GET':
      if current_user.is_authenticated:
        if be.is_verified(user_id=current_user.user_id):
            ...
            notifications = be.getNotifcations(user_id=current_user.user_id)
            return render_template('verify.html', logged_in = current_user.is_authenticated, title="Verify", email=current_user.email, notifications=notifications)
      else:
            notifications = be.getNotifcations(user_id=current_user.user_id)
            return render_template('verify.html', logged_in = current_user.is_authenticated, title="Verify", email=current_user.email, notifications=notifications)
    if request.method == 'POST':
        if current_user.is_authenticated:
            ...
            if val == True:
             ...
             notifications = be.getNotifcations(user_id=current_user.user_id)
             return render_template('verify.html', logged_in = current_user.is_authenticated, title="Verify", email=current_user.email, alert="Incorrect Code", notifications=notifications)
        ...
        if current_user.is_authenticated:
           if be.is_woftheday_stack_available():
            ...
            return redirect('/news/read')
             ...
             notifications = be.getNotifcations(user_id=current_user.user_id)
             return render_template('wordofthedaywrite.html', logged_in = current_user.is_authenticated, title="Word Of The Day | Write | lastevents.space", value=False, notifications=notifications)
        else:
            return "Unauthorized"
    else:
        return "Invalid Request"
...
def contact():
    if request.method == 'POST':
        ...
        if email.isspace() or message.isspace():
           if current_user.is_authenticated: 
            notifications = be.getNotifcations(user_id=current_user.user_id)
            return render_template('contact.html', logged_in = current_user.is_authenticated, alert="Fields should not be empty", welcome=True, title="Contact | lastevents.space", notifications=notifications)
        ...
        if current_user.is_authenticated: 
         notifications = be.getNotifcations(user_id=current_user.user_id)
         return render_template('contact.html', logged_in = current_user.is_authenticated, alert="Message sent successfully", welcome=True, title="Contact | lastevents.space", notifications=notifications)
    ...
    if current_user.is_authenticated:
     notifications = be.getNotifcations(user_id=current_user.user_id)
     return render_template('contact.html', logged_in = current_user.is_authenticated, contact__="#8479794f", welcome=True, title="Contact | lastevents.space", notifications=notifications)
...
def search():
    ...
    if current_user.is_authenticated:
        notifications = be.getNotifcations(user_id=current_user.user_id)
        return render_template("search.html", title="Search | lastevents.space", search__="#8479794f", logged_in = current_user.is_authenticated, notifications=notifications)
...
@app.route('/develop/view')
def developview():
    comments = be.getCommentsofArticle(article_id="Lesson-from-the-book-of-history")
    return render_template("view1.html", comments=comments, logged_in = current_user.is_authenticated)
